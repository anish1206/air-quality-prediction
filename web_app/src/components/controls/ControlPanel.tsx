...
const LAYERS: { key: 'edges' | 'pulses' | 'plumes'; label: string }[] = [
  { key: 'edges', label: 'Causal Edges' },
  { key: 'pulses', label: 'Cascade Pulses' },
  { key: 'plumes', label: 'Atmospheric Plumes' },
];
          ...
          {LAYERS.map((layer) => {
            ...
            return (
              <button
                ...
                className="flex items-center justify-between px-2 py-1.5 rounded-[8px] text-[12px] text-white/70 hover:bg-white/[0.04]"
              >
                <span>{layer.label}</span>
                <span
                  className="w-8 h-[16px] rounded-full relative transition-colors"
